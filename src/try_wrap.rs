...
///
/// Usage is identical to [`wrap()`](textwrap::wrap()).
...
fn try_wrap_single_line_slow_path(
    line: &str,
    options: &Options<'_>,
    counts: &mut Vec<usize>,
) {
    ...
    let wrapped_words =
        options.wrap_algorithm.wrap(&broken_words, &line_widths);
    ...
    for words in wrapped_words {
        ...
        let mut width = words
            .iter()
            .map(|word| {
                (word.width() as usize) + (word.whitespace_width() as usize)
            })
            .sum::<usize>()
...
mod tests {
    ...
    macro_rules! dw_vec {
        ( $( $x:expr ),* $(,)? ) => {
            vec![$( textwrap::core::display_width($x) ),*]
        };
    }
    ...
    fn indent_multiple_lines() {
        let options =
            Options::new(6).initial_indent("* ").subsequent_indent("  ");
        assert_eq!(
    ...
    fn simple_hyphens() {
        let options =
            Options::new(8).word_splitter(WordSplitter::HyphenSplitter);
        assert_eq!(
            try_wrap("foo bar-baz", &options),
            dw_vec!["foo bar-", "baz"]
        );
    ...
    fn no_hyphenation() {
        let options =
            Options::new(8).word_splitter(WordSplitter::NoHyphenation);
        assert_eq!(
            try_wrap("foo bar-baz", &options),
            dw_vec!["foo", "bar-baz"]
        );
